...
function defaultServicesState() {
  return {
    frontend: { ok: false, statusCode: 0, label: 'PENDING' },
    auth: { ok: false, statusCode: 0, label: 'PENDING' },
    task: { ok: false, statusCode: 0, label: 'PENDING' },
    user: { ok: false, statusCode: 0, label: 'PENDING' }
  };
}
function normalizeLegacyHealthStateOnce() {
  const versionKey = 'strict_health_v633';
  if (getMetaValue(versionKey) === '1') return;
  const rows = db.prepare('SELECT id FROM groups').all();
  const reset = db.prepare('UPDATE groups SET services_json = ?, last_checked_at = ?, updated_at = ? WHERE id = ?');
  const now = nowIso();
  const tx = db.transaction(() => {
    for (const row of rows) {
      reset.run(JSON.stringify(defaultServicesState()), '', now, row.id);
    }
    setMetaValue(versionKey, '1');
  });
  tx();
}
normalizeLegacyHealthStateOnce();


const insertAuditStmt = db.prepare(`
  INSERT INTO audit_logs (actor_role, actor_username, group_id, action, area, summary, details_json, created_at)
  VALUES (@actor_role, @actor_username, @group_id, @action, @area, @summary, @details_json, @created_at)
`);
const selectAuditByGroupStmt = db.prepare(`
  SELECT * FROM audit_logs WHERE group_id = ? ORDER BY datetime(created_at) DESC, id DESC LIMIT ?
`);
const selectAuditRecentStmt = db.prepare(`
  SELECT * FROM audit_logs ORDER BY datetime(created_at) DESC, id DESC LIMIT ?
`);

function logAudit({ actorRole, actorUsername, groupId = '', action, area, summary, details = {} }) {
  insertAuditStmt.run({
    actor_role: String(actorRole || 'system'),
    actor_username: String(actorUsername || 'system'),
...
function getGroupHistory(groupId, role = 'teacher', limit = 20) {
  const rows = selectAuditByGroupStmt.all(groupId, limit).map(mapAuditRow);
  if (role === 'teacher') return rows;
  return rows.filter((row) => row.area !== 'teacher_private');
}

function getRecentAudit(limit = 30) {
  return selectAuditRecentStmt.all(limit).map(mapAuditRow);
}
...
function toPublicGroup(row, viewerRole = 'teacher') {
  ...
  const services = { ...defaultServicesState(), ...safeJsonParse(row.services_json, {}) };
  const urls = {
...
function toBoardGroup(g) {
  return {
    ...
    urlsCount: g.urlsCount,
    docsCount: g.docsCount,
    repoCount: g.repoCount,
    readiness: g.readiness,
    readinessLevel: g.readinessLevel,
    startState: g.startState,
    studentStatus: g.studentStatus,
    teacherStatus: g.teacherStatus,
    services: g.services,
    servicesOk: g.servicesOk,
    hasRecentUpdate: g.hasRecentUpdate,
    updatedAt: g.updatedAt,
    lastStudentUpdateAt: g.lastStudentUpdateAt,
    lastCheckedAt: g.lastCheckedAt
  };
}
function getBoardGroups() {
  return getAllGroups('teacher').map(toBoardGroup);
}
...
function httpCheck(kind, targetUrl) {
  return new Promise((resolve) => {
    ...
    const req = lib.request(targetUrl, {
      ...
      timeout: 6000,
      headers: {
        'User-Agent': 'ENGSE207-Monitor/1.0',
        'Accept': 'application/json, text/html;q=0.9, */*;q=0.8'
      }
    }, (resp) => {
      let body = '';
      resp.setEncoding('utf8');
      resp.on('data', (chunk) => { body += chunk; if (body.length > 65536) body = body.slice(0, 65536); });
      resp.on('end', () => {
        ...
        if (kind === 'frontend') {
          ...
          const hasHtmlMarker = bodyLower.includes('name="engse207-frontend-health"') && bodyLower.includes('content="frontend-ok"');
          const hasHeaderMarker = headerMarker === FRONTEND_HEALTH_MARKER;
          const ok = statusCode >= 200 && statusCode < 400 && (hasHeaderMarker || hasHtmlMarker);
          let label = 'NO MARKER';
          if (ok) label = 'OK';
          else if (statusCode >= 400) label = `HTTP ${statusCode}`;
          else if (!contentType.includes('text/html')) label = 'NOT HTML';
...
function serveStatic(req, res, pathname) {
  const spaRoutes = new Set(['/', '/board', '/login', '/teacher', '/student']);
  const rel = spaRoutes.has(pathname) ? '/index.html' : pathname;
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (pathname === '/api/board' && req.method === 'GET') {
      return sendJson(res, 200, { items: getBoardGroups(), meta: getMetaPayload() });
    }
    ...
    if (pathname === '/api/auth/login' && req.method === 'POST') {
      ...
      if (user.role === 'student') {
        markStudentLoggedIn(user.groupId);
        logAudit({ actorRole: 'student', actorUsername: user.username, groupId: user.groupId, action: 'LOGIN', area: 'auth', summary: `Student login: ${user.username}` });
      } else {
        logAudit({ actorRole: 'teacher', actorUsername: user.username, groupId: '', action: 'LOGIN', area: 'auth', summary: `Teacher login: ${user.username}` });
      }
      return sendJson(res, 200, { user }, { 'Set-Cookie': `sid=${encodeURIComponent(sid)}; HttpOnly; Path=/; SameSite=Lax` });
    }
    if (pathname === '/api/auth/logout' && req.method === 'POST') {
      const session = getSession(req);
      if (session) logAudit({ actorRole: session.user.role, actorUsername: session.user.username, groupId: session.user.groupId || '', action: 'LOGOUT', area: 'auth', summary: `Logout: ${session.user.username}` });
      clearSession(req);
    ...
    if (pathname === '/api/check-all' && req.method === 'POST') {
      ...
      const teacher = requireTeacher(req).user;
      logAudit({ actorRole: 'teacher', actorUsername: teacher.username, groupId: '', action: 'CHECK_ALL_HEALTH', area: 'health', summary: 'Teacher checked health for all groups' });
    ...
    if (mGroupCheck && req.method === 'POST') {
      requireTeacher(req);
      const id = decodeURIComponent(mGroupCheck[1]);
      const item = await checkGroupHealth(id);
      const teacher = requireTeacher(req).user;
      logAudit({ actorRole: 'teacher', actorUsername: teacher.username, groupId: id, action: 'CHECK_HEALTH', area: 'health', summary: `Teacher checked health for ${id}`, details: { servicesOk: item.servicesOk, lastCheckedAt: item.lastCheckedAt } });
    ...
    if (mReview && req.method === 'PATCH') {
      ...
      const teacher = requireTeacher(req).user;
      logAudit({ actorRole: 'teacher', actorUsername: teacher.username, groupId: id, action: 'REVIEW_SAVE', area: 'teacher_review', summary: `Teacher updated review for ${id}`, details: { teacherStatus: item.teacherStatus, systemScore: item.systemScore, docsScore: item.docsScore, interviewScore: item.interviewScore, bonusScore: item.bonusScore } });
      return sendJson(res, 200, { item });
    }
    ...
    const mHistory = pathname.match(/^\/api\/groups\/([^/]+)\/history$/);
    if (mHistory && req.method === 'GET') {
      requireTeacher(req);
      const id = decodeURIComponent(mHistory[1]);
      return sendJson(res, 200, { items: getGroupHistory(id, 'teacher', 30) });
    }

    if (pathname === '/api/me/group/history' && req.method === 'GET') {
      const session = requireStudent(req);
      return sendJson(res, 200, { items: getGroupHistory(session.user.groupId, 'student', 20) });
    }
    ...
    if (pathname === '/api/me/group/setup' && req.method === 'PATCH') {
      ...
      logAudit({ actorRole: 'student', actorUsername: session.user.username, groupId: item.id, action: 'SETUP_GROUP', area: 'student_setup', summary: `Student setup/updated members for ${item.id}`, details: { memberCount: item.memberCount, groupName: item.groupName } });
    ...
    if (pathname === '/api/me/group/status' && req.method === 'PATCH') {
      ...
      logAudit({ actorRole: 'student', actorUsername: session.user.username, groupId: item.id, action: 'CHANGE_STATUS', area: 'student_status', summary: `Student changed status for ${item.id}`, details: { studentStatus: item.studentStatus } });
      return sendJson(res, 200, { item: getGroupById(item.id, 'student') });
    }
    if (pathname === '/api/me/group/note' && req.method === 'PATCH') {
      ...
      logAudit({ actorRole: 'student', actorUsername: session.user.username, groupId: item.id, action: 'UPDATE_NOTE', area: 'student_note', summary: `Student updated note for ${item.id}` });
      return sendJson(res, 200, { item: getGroupById(item.id, 'student') });
    }
...
server.listen(PORT, () => {
  console.log(`Set2 monitor Phase 7 running at http://localhost:${PORT}`);
});
