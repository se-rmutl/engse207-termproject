...
function loginGroup(username, password) {
  ...
  const group = db.prepare('SELECT * FROM groups WHERE id = ?').get(acc.group_id);
  ...
  return new Promise((resolve) => {
    ...
    const req = lib.request(targetUrl, { method: 'GET', timeout: 6000 }, (resp) => {
      ...
      resp.on('end', () => {
        ...
        if (kind === 'frontend') {
          const ok = statusCode >= 200 && statusCode < 400 && (contentType.includes('text/html') || body.trim().length > 0);
        ...
        const serviceOk = !expectedService || !parsedJson?.service || String(parsedJson.service) === expectedService;
...
function studentOwnGroup(session) { return getGroupById(session.user.groupId, 'student'); }
