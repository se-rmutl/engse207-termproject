...
const sseClients = new Set(); // { res, session }
...
function seedGroupsAndAccounts() {
  ...
  const insertGroup = db.prepare(`
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      group_name=CASE
        WHEN groups.group_name IS NULL OR groups.group_name = '' OR groups.group_name GLOB 'SEC[12] GROUP[0-9][0-9]' THEN excluded.group_name
    ...
    for (const sec of ['sec1', 'sec2']) {
      for (let i = 1; i <= 20; i += 1) {
        ...
        insertGroup.run({
          ...
          group_name: `${sec.toUpperCase()} ${groupCode.toUpperCase()}`,
          updated_at: now
        });
        insertAccount.run({
          username: id,
...
function canonicalGroupId(username) {
  const normalized = String(username || '').trim().toLowerCase();
  return /^sec[12]-group\d{2}$/.test(normalized) ? normalized : null;
}
function loginGroup(username, password) {
  const normalized = canonicalGroupId(username) || String(username || '').trim().toLowerCase();
  const acc = db.prepare('SELECT * FROM group_accounts WHERE username = ?').get(normalized);
  if (!acc) return null;
  if (acc.password_hash !== hashPassword(password)) return null;
  const expectedGroupId = canonicalGroupId(normalized) || acc.group_id;
  const group = db.prepare('SELECT * FROM groups WHERE id = ?').get(expectedGroupId) || db.prepare('SELECT * FROM groups WHERE id = ?').get(acc.group_id);
  if (!group) return null;
  if (acc.group_id !== group.id) {
    db.prepare('UPDATE group_accounts SET group_id = ?, updated_at = ? WHERE username = ?').run(group.id, nowIso(), normalized);
  }
  return {
    role: 'student',
    username: normalized,
...
function httpCheck(targetUrl) {
  return new Promise((resolve) => {
    if (!targetUrl) return resolve({ ok: false, statusCode: 0, label: 'N/A' });
    let parsed;
    try { parsed = new URL(targetUrl); } catch { return resolve({ ok: false, statusCode: 0, label: 'BAD URL' }); }
    const lib = parsed.protocol === 'https:' ? require('https') : require('http');
    const req = lib.request(targetUrl, { method: 'GET', timeout: 6000 }, (resp) => {
      const ok = resp.statusCode >= 200 && resp.statusCode < 400;
      resp.resume();
      resolve({ ok, statusCode: resp.statusCode || 0, label: ok ? 'OK' : `HTTP ${resp.statusCode || 0}` });
    });
...
async function checkGroupHealth(id) {
  ...
  const [frontend, auth, task, user] = await Promise.all([
    httpCheck(checks.frontend), httpCheck(checks.auth), httpCheck(checks.task), httpCheck(checks.user)
  ]);
...
function writeCsv(groups) {
  const headers = ['id','section','groupCode','groupName','startState','memberCount','urlsCount','docsCount','studentStatus','teacherStatus','systemScore','docsScore','interviewScore','bonusScore','readiness','frontendUrl','authUrl','taskUrl','userUrl'];
  const esc = (v) => {
    const s = String(v ?? '');
    return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s;
  };
  return [headers.join(','), ...groups.map((g) => [
    g.id, g.section, g.groupCode, g.groupName, g.startState, g.memberCount, g.urlsCount, g.docsCount, g.studentStatus, g.teacherStatus,
    g.systemScore, g.docsScore, g.interviewScore, g.bonusScore, g.readiness, g.urls.frontend, g.urls.auth, g.urls.task, g.urls.user
  ].map(esc).join(','))].join('\n');
}
...
function writeSse(client, type, payload) {
...
function broadcast(type, payload) {
  for (const client of sseClients) writeSse(client, type, payload);
}
function buildSnapshotPayload(session) {
  if (session?.user?.role === 'teacher') return { items: getAllGroups('teacher'), meta: getMetaPayload() };
  const item = studentOwnGroup(session);
  return { item, items: item ? [item] : [], meta: getMetaPayload() };
}
function broadcastSnapshot() {
  for (const client of sseClients) writeSse(client, 'snapshot', buildSnapshotPayload(client.session));
}
function broadcastMeta() {
  for (const client of sseClients) writeSse(client, 'meta', { meta: getMetaPayload() });
}
...
function studentOwnGroup(session) {
  const canonicalId = canonicalGroupId(session?.user?.username);
  const groupId = canonicalId || session?.user?.groupId;
  if (!groupId) return null;
  const item = getGroupById(groupId, 'student');
  if (item && session?.user && session.user.groupId !== item.id) {
    session.user.groupId = item.id;
    session.user.section = item.section;
    session.user.groupCode = item.groupCode;
    session.user.displayName = item.groupName;
  }
  return item;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (pathname === '/api/stream') {
      ...
      res.write(`event: snapshot\ndata: ${JSON.stringify(buildSnapshotPayload(session))}\n\n`);
