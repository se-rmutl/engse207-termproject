...
function broadcastSnapshot() { broadcast('snapshot', { items: getAllGroups('teacher'), meta: getMetaPayload() }); }
function broadcastMeta() { broadcast('meta', { meta: getMetaPayload() }); }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (pathname === '/api/stream') {
      ...
      const items = session.user.role === 'teacher' ? getAllGroups('teacher') : [studentOwnGroup(session)];
      res.write(`event: snapshot\ndata: ${JSON.stringify({ items, meta: getMetaPayload() })}\n\n`);
      sseClients.add(res);
