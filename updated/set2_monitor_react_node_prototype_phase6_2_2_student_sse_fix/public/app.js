...
function timeValue(v) { return v ? new Date(v).getTime() : 0; }
    ...
    <div className="login-shell">
      <div className="login-card panel">
        <div className="login-head">
          <div className="pill">Phase 6.2</div>
          <h1>ENGSE207 Set 2 Monitor</h1>
          <div className="sub">Bugfix + workflow polish</div>
        </div>
...
function TeacherDashboard({ user, onLogout }) {
  ...
  const filtered = useMemo(() => groups.filter((g) => {
    ...
    const recentDelta = Number(!!b.hasRecentUpdate) - Number(!!a.hasRecentUpdate);
    if (recentDelta) return recentDelta;
    const updateDelta = timeValue(b.lastStudentUpdateAt || b.updatedAt) - timeValue(a.lastStudentUpdateAt || a.updatedAt);
    if (updateDelta) return updateDelta;
    return a.groupName.localeCompare(b.groupName);
  }), [groups, search, sectionFilter]);

  const latestUpdated = useMemo(() => filtered.find((g) => g.hasRecentUpdate) || null, [filtered]);
        ...
        <div className="panel">
          <div className="panel-head">
            ...
            <div className="footer-actions">
              ...
              {latestUpdated ? <button onClick={() => { setSelectedId(latestUpdated.id); setDirty(false); }}>อัปเดตล่าสุด</button> : null}
...
function StudentDashboard({ user, onLogout }) {
  ...
  const editingLock = setupDirty || submissionDirty;
  ...
  useEffect(() => { loadMine().catch((e) => setMessage(e.message)); }, []);
  useEffect(() => {
    const es = new EventSource('/api/stream');
    es.addEventListener('snapshot', (e) => {
      const payload = JSON.parse(e.data || '{}');
      const list = Array.isArray(payload.items) ? payload.items.filter(Boolean) : [];
      const exact = list.find((g) => g && (g.id === user.groupId || (user.username && g.id === user.username) || (g.section === user.section && g.groupCode === user.groupCode)));
      const single = list.length === 1 ? list[0] : null;
      const item = payload.item || exact || single || null;

      setStreamState({ mode: 'open' });

      // ถ้า stream ส่งข้อมูลไม่ตรงกลุ่มของผู้ใช้ ให้ดึง /api/me/group ใหม่แทนเพื่อกันหน้าจอเด้งไปกลุ่มแรก
      if (!item || item.id !== (user.groupId || user.username)) {
        if (!editingLock) loadMine().catch((err) => setMessage(err.message));
        return;
      }
    ...
    es.onerror = () => setStreamState({ mode: 'fallback' });
    return () => es.close();
  }, [editingLock, user.groupCode, user.groupId, user.section, user.username]);
  useEffect(() => {
    if (streamState.mode !== 'fallback') return undefined;
    const t = setInterval(() => { if (!editingLock) loadMine(); }, FALLBACK_POLL_MS);
    return () => clearInterval(t);
  }, [streamState.mode, editingLock]);

  const selected = draft || group;
  const displayTotal = selected ? ((selected.systemScore || 0) + (selected.docsScore || 0) + (selected.interviewScore || 0)) : 0;
  const lastVisibleUpdate = selected?.lastTeacherReviewAt || selected?.lastStudentUpdateAt || selected?.updatedAt || '';
  function upd(k, v) { setSubmissionDirty(true); setDraft((p) => ({ ...(p || {}), [k]: v })); }
  function updGroupName(v) { setSetupDirty(true); setDraft((p) => ({ ...(p || {}), groupName: v })); }
  function updateUrl(key, value) { setSubmissionDirty(true); setDraft((p) => ({ ...(p || {}), urls: { ...(p?.urls || {}), [key]: value } })); }
  function updateDoc(key, value) { setSubmissionDirty(true); setDraft((p) => ({ ...(p || {}), docs: { ...(p?.docs || {}), [key]: value } })); }
  function markMembersDirty(next) { setSetupDirty(true); setMembers(next); }

  async function saveSetup() {
    if (!selected) return;
    setSetupSaving(true);
    try {
      const res = await api('/api/me/group/setup', {
        method: 'PATCH',
        body: JSON.stringify({ members, groupName: selected.groupName })
      });
      setGroup(res.item);
      setDraft((prev) => ({
        ...(prev || {}),
        ...res.item,
        urls: prev?.urls || res.item.urls,
        docs: prev?.docs || res.item.docs,
        studentNote: prev?.studentNote ?? res.item.studentNote,
        studentStatus: prev?.studentStatus ?? res.item.studentStatus
      }));
  ...
  async function saveSubmission() {
    ...
    try {
      ...
      const item = statusRes.item || subRes.item;
      setGroup(item);
      setDraft((prev) => ({ ...item, groupName: prev?.groupName || item.groupName }));
      setSubmissionDirty(false);
      setMessage('บันทึกการส่งงานแล้ว');
    } catch (e) {
      setMessage(e.message);
    } finally {
      ...
      <div className="panel">
        ...
        <div className="panel-body">
          <div className="summary student-cards">
            ...
            <div className="metric"><div className="k">คะแนนรวม</div><div className="v">{displayTotal}<small>/100</small></div><div className="tiny">+{selected.bonusScore || 0}</div></div>
          </div>

          <div className="save-state-row">
            <span className={`badge ${editingLock ? 'warn' : 'ready'}`}>{editingLock ? 'มีข้อมูลยังไม่บันทึก' : 'ข้อมูลล่าสุดถูกบันทึกแล้ว'}</span>
            <span className="small-muted">อัปเดตล่าสุด: {fmtDate(lastVisibleUpdate)}</span>
          </div>
          ...
          <div className="student-detail-grid">
            <div className="panel student-panel">
              ...
              <div className="panel-body section">
                <div className="field"><label className="field-label">ชื่อกลุ่มแสดงผล</label><input value={selected.groupName || ''} onChange={(e) => updGroupName(e.target.value)} /></div>
                <MemberFields members={members} setMembers={markMembersDirty} />
                <div className="footer-actions">
                  <button className="primary" onClick={saveSetup}>{setupSaving ? 'กำลังบันทึก...' : 'บันทึกข้อมูลสมาชิก'}</button>
                </div>
              </div>
            </div>
            ...
            <div className="panel student-panel sticky-panel">
              ...
              <div className="panel-body section">
                ...
                {message ? <div className="pill">{message}</div> : null}
                {!selected.initialized ? <div className="note"><b>หมายเหตุ</b><br />คุณยังสามารถบันทึก URLs, เอกสาร และสถานะได้ แต่ควรบันทึกข้อมูลสมาชิกอย่างน้อย 2 คนเพื่อให้ระบบถือว่าเริ่มทำงานแล้ว</div> : null}
                <div className="footer-actions">
                  <button className="primary" onClick={saveSubmission}>{submissionSaving ? 'กำลังบันทึก...' : 'บันทึกการส่งงาน'}</button>
                </div>
              </div>
            </div>
          </div>
...
function App() {
  ...
  async function logout() { try { await api('/api/auth/logout', { method: 'POST' }); } catch {} setUser(null); }
  ...
  return user.role === 'teacher' ? <TeacherDashboard user={user} onLogout={logout} /> : <StudentDashboard user={user} onLogout={logout} />;
}
